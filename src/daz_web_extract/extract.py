...
BYPASS_TIER2_CODES = set(range(400, 600)) - {403, 429}
...
async def extract(url: str, max_tier: int = 4) -> ExtractionResult:
    ...
    if _should_bypass_tier2(tier1_result) and max_tier >= 3:
...
# ##################################################################
# should bypass tier2
# http 4xx/5xx (except 403 and 429) bypass tier 2 and go straight to
# playwright because the server explicitly rejected the request
def _should_bypass_tier2(result: ExtractionResult) -> bool:
    if result.status_code is not None and result.status_code in BYPASS_TIER2_CODES:
        return True
    return False
