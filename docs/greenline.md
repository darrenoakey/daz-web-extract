# greenline — daz-web-extract

This repo uses **greenline**: a local, serialized, gated CI/CD tool. There is one
canonical checkout of `daz-web-extract`; it is always on `main`, always clean,
and always identical to what prod (`daz-web-extract`) runs. Every change reaches
...
```
greenline worktree feature-x        # new worktree at /Volumes/Gumby/worktrees/greenline/daz-web-extract/feature-x
cd /Volumes/Gumby/worktrees/greenline/daz-web-extract/feature-x         # branch gl/feature-x off last-green
# ...edit, test, commit...
...
- **`./run deploy`** — cwd = the canonical checkout. Rebuild/restart prod (e.g.
  `auto -q restart daz-web-extract`), **health-check**, exit nonzero on unhealthy, and be
